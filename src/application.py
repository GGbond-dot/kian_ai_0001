...
class Application:
    ...
    async def run(self, *, protocol: str = "websocket", mode: str = "gui") -> int:
        ...
        try:
            ...
            try:
                ...
                pass
            # 插件：start（UIPlugin 在此展示窗口）
            await self.plugins.start_all()
            # local 协议：UI 就绪后自动连接并进入持续监听
            if protocol == "local":
    ...
    async def start_listening_manual(self) -> None:
        try:
            ...
            if not ok:
                return
            # 本地协议：保持持续对话模式，不重置 keep_listening
            from src.protocols.local_agent_protocol import LocalAgentProtocol
            if not isinstance(self.protocol, LocalAgentProtocol):
                self.keep_listening = False
    ...
    async def _on_audio_channel_opened(self):
        logger.info("协议通道已打开")
        # 本地协议：自动进入持续对话模式（说完自动继续监听，无需每次手动按键）
        from src.protocols.local_agent_protocol import LocalAgentProtocol
