...
class MusicPlayer:
    ...
    async def stop(self) -> dict:
        ...
        try:
            ...
            # 停止 ffplay 进程（PulseAudio 路径）
            if hasattr(self, "_ffplay_proc") and self._ffplay_proc and self._ffplay_proc.returncode is None:
    ...
    async def pause(self, source: str = "manual") -> dict:
        ...
        try:
            ...
            # ffplay 路径：发送 SIGSTOP 暂停进程
            if hasattr(self, "_ffplay_proc") and self._ffplay_proc and self._ffplay_proc.returncode is None:
                import signal
                self._ffplay_proc.send_signal(signal.SIGSTOP)
    ...
    async def resume(self) -> dict:
        ...
        try:
            ...
            # ffplay 路径：发送 SIGCONT 恢复进程
            if hasattr(self, "_ffplay_proc") and self._ffplay_proc and self._ffplay_proc.returncode is None:
                import signal
                self._ffplay_proc.send_signal(signal.SIGCONT)
                self.paused = False
    ...
    async def _start_playback(
        ...
        try:
            ...
            # ── 检测 PulseAudio，优先用 ffplay 直接播放（WSL2/Linux）──
            _use_ffplay = False
            try:
                import subprocess as _sp
                _r = _sp.run(["pactl", "info"], capture_output=True, timeout=2)
                _use_ffplay = (_r.returncode == 0)
            except Exception:
                pass

            if _use_ffplay:
                self._playback_task = asyncio.create_task(
                    self._playback_loop_ffplay(file_path, start_position)
                )
            else:
                # 原 AudioCodec 路径
    ...
    async def _playback_loop_ffplay(self, file_path: Path, start_position: float = 0.0):
        """PulseAudio 环境下直接用 ffplay 播放（WSL2 sounddevice 不发声时的替代路径）"""
        cmd = ["ffplay", "-autoexit", "-nodisp", "-loglevel", "quiet"]
        if start_position > 0:
            cmd.extend(["-ss", str(start_position)])
        cmd.append(str(file_path))
        try:
            self._ffplay_proc = await asyncio.create_subprocess_exec(
                *cmd,
                stdout=asyncio.subprocess.DEVNULL,
                stderr=asyncio.subprocess.DEVNULL,
            )
            logger.info(f"[ffplay] 开始播放: {file_path.name}")
            await self._ffplay_proc.wait()
            if self.is_playing:
                logger.info("音乐播放完成")
                await self._handle_playback_finished()
        except asyncio.CancelledError:
            if hasattr(self, "_ffplay_proc") and self._ffplay_proc:
                self._ffplay_proc.terminate()
        except Exception as e:
            logger.error(f"[ffplay] 播放异常: {e}")
