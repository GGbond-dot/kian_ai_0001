...
| memory_store 持久化 | ✅ 已掌握(85 分) | `src/llm/memory_store.py`、`src/llm/agent.py` |
| asyncio 事件循环第一层 | ✅ 已掌握一轮 | `main.py`、`src/application.py`、`src/protocols/local_agent_protocol.py` |

### 3.2 优先级 P0(当前主线)

| 技术点 | 主要代码位置 | 学习重点 |
|---|---|---|
| asyncio 事件循环第二层 | `src/application.py`、`src/protocols/local_agent_protocol.py`、`src/display/slam_bridge.py`、`src/plugins/shortcuts.py` | `asyncio.Event`、`asyncio.Lock`、并发保护、shutdown、跨线程调度复盘 |

#### asyncio 已学边界(不要重复讲)

- `qasync.QEventLoop` vs `asyncio.run`: GUI 模式用 qasync 整合 Qt 事件循环和 asyncio 协程; CLI/Web 使用标准 `asyncio.run`
- `Application.run`: 保存主 loop、初始化协议、注册/启动插件、local 模式自动进入监听、`_wait_shutdown` 维持主协程不退出
- `await` / `create_task` / `cancel` / `CancelledError + raise`: 已能讲清当前协程等待、后台 Task、取消请求、取消清理与重新抛出
- `run_in_executor` / `asyncio.to_thread`: 用于同步阻塞函数,如 `stt.preload`
- `run_coroutine_threadsafe`: 用于从 ROS/MQTT/快捷键等非 asyncio 线程安全提交协程回主 loop

#### asyncio 下一步问题

- `connect_protocol()` 为什么用 `_connect_lock`
- `set_device_state()` 为什么用 `_state_lock`,以及为什么锁外广播插件通知
- `_shutdown_event` 如何让主协程等待退出信号
- `Application.spawn()` 如何统一管理后台任务、异常日志和 shutdown cancel
