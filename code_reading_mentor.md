...
- **项目名**:智能终端机器人(基于开源项目 py-xiaozhi 二次开发)
...
| 技术点 | 主要代码位置 | 学习重点 |
|---|---|---|
| ReAct Agent + Tool Calling | `src/llm/agent.py`、`src/protocols/local_agent_protocol.py` | LLM tool calling 协议、ReAct 循环、conversation_history 构造、最大轮数限制 |
| MCP 工具体系 | `src/mcp/tools/` 整个目录,重点 `robot_dispatch/tools.py` 和 `manager.py` | 工具注册机制、参数校验、白名单设计 |
| memory_store 持久化 | `src/llm/memory_store.py` | 对话记忆持久化、session 管理 |
